...
const { pipelineStatuses, taskStatuses } = require('@distributedkube/consts');
...
const { uuid } = require('@distributedkube/uid');
    ...
    return {
        ...
        storageAdapters: {
            s3: {
                ...
                moduleName: process.env.STORAGE_MODULE || '@distributedkube/s3-adapter'
            }
        },
        kubernetes: {
...
describe('consumer tests', () => {
    ...
    afterEach(async function () {
        ...
        await storageManager.delete({
            path: 'local-distributedkube-algo-metrics'
        });
    });
    ...
    it('Check algo metrics are uploaded', async () => {
        ...
        const uploadedFiles = await storageManager.list({ path: 'local-distributedkube-algo-metrics/pipeName/A/' });
    ...
    it('store data and validate result from algorithm', async () => {
        const config = getConfig();
        storageManager.distributedkube.put({ jobId: config.jobId, taskId: config.taskId, data: { data: { engine: 'deep' } } }).then(async (link) => {
    ...
    xit('finish job and store data - object with path', () => {
        return (new Promise(async function (resolve, reject) {
            ...
            const arr = [];
            arr.push(storageManager.distributedkube.put({ jobId: config.jobId, taskId: '1', data: { field: { test: 'test-extract1' } } }))
            arr.push(storageManager.distributedkube.put({ jobId: config.jobId, taskId: '2', data: { field: { test: 'test-extract2' } } }))
            arr.push(storageManager.distributedkube.put({ jobId: config.jobId, taskId: '3', data: { field: { test: 'test-extract3' } } }))
