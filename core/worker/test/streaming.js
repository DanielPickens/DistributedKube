...
const pipeline = {
    ...
    flowInputMetadata: {
        ...
        storageInfo: {
            "path": "local-distributedkube/main:streaming:9dy12jfh/main:streaming:9dy12jfh"
        }
    }
}
...
describe('Streaming', () => {
    ...
    describe('scale-up', () => {
        ...
        it('should scale only up based on req/res rate with a maxStatelessCount limit', async () => {
            const scale = async (data) => {
                data[0].sent += 10;
                data[0].responses += 3;
                streamService.reportStats(data);
