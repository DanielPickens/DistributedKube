...
describe('StorageHelper', () => {
    describe('StorageV1', () => {
        ...
        xit('store data and validate extraction no cache', async () => {
            ...
            const link1 = await storageManager.distributedkube.put({ jobId: config.jobId, taskId: taskId1, data: { data: { engine: 'deep' } } });
            const link2 = await storageManager.distributedkube.put({ jobId: config.jobId, taskId: taskId2, data: { myValue: 1973 } });
            const data = {
        ...
        it('store data and validate extraction using cache', async () => {
            const config = getConfig();
            const link = await storageManager.distributedkube.put({ jobId: config.jobId, taskId: config.taskId, data: { data: { engine: 'deep' } } });
            const link2 = await storageManager.distributedkube.put({ jobId: config.jobId, taskId: "5", data: { myValue: 1973 } });
            const data = {
        ...
        it('store data and validate extraction reseting cache', async () => {
            ...
            const link = await storageManager.distributedkube.put({ jobId: config.jobId, taskId: config.taskId, data: { data: { engine: 'deep' } } });
            const link2 = await storageManager.distributedkube.put({ jobId: config.jobId, taskId: "5", data: { myValue: 1973 } });
            const link3 = await storageManager.distributedkube.put({ jobId: config.jobId, taskId: "6", data: { myOtherValue: 1010 } });
