...
const KubernetesClient = require('@distributedkube/kubernetes-client').Client;
...
class KubernetesApi {
    ...
    async getWorkerJobs() {
        const jobsRaw = await this._client.jobs.get({ labelSelector: `type=${CONTAINERS.WORKER},group=distributedkube` });
        return jobsRaw;
    }
    ...
    async getVersionsConfigMap() {
        const res = await this._client.configMaps.get({ name: 'distributedkube-versions' });
    ...
    async _getNamespacedWithNodeListResources() {
        ...
        const nodesConfigMap = await this._client.configMaps.get({ name: 'distributedkube-nodes' });
        let nodes = { items: [] };
