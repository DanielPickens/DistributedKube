...
const { randomString } = require('@distributedkube/uid');
...
const { applyResourceRequests, applyEnvToContainer, applyNodeSelector, applyImage,
    ...
    applyImagePullSecret } = require('@distributedkube/kubernetes-client').utils;
...
const applyDevMode = (inputSpec, { algorithmOptions = {}, algorithmName, clusterOptions = {} }) => {
    ...
    let devMountPath = '/distributedkube/algorithm-runner/algorithm_unique_folder';
    if (!devMode) {
        return inputSpec;
    }
    ...
    spec = applyVolumeMounts(spec, CONTAINERS.ALGORITHM, {
        name: 'distributedkube-dev-sources',
    ...
    spec = applyVolumes(spec, {
        name: 'distributedkube-dev-sources',
        persistentVolumeClaim: {
            claimName: 'distributedkube-dev-sources-pvc'
        }
    });
    return spec;
};
...
const applyDataSourcesVolumes = (inputSpec, clusterOptions) => {
    ...
    spec = applyVolumeMounts(spec, CONTAINERS.ALGORITHM, {
        ...
        mountPath: '/distributedkube/datasources-storage'
    });
    spec = applyVolumes(spec, {
        name: 'datasources-storage',
        persistentVolumeClaim: {
            claimName: 'distributedkube-datasources'
        }
    });
    return spec;
};
...
const applyDatascienceMetricsVolumes = (inputSpec, dashboardEnabled) => {
    ...
    spec = applyVolumeMounts(spec, CONTAINERS.ALGORITHM, {
        ...
        mountPath: '/distributedkube/datasciencemetrics-storage'
    });
    if (dashboardEnabled) {
        spec = applyVolumes(spec, {
            name: 'datasciencemetrics-storage',
            persistentVolumeClaim: {
                claimName: 'distributedkube-datasciencemetrics'
            }
        });
    }
    else {
...
const applyLogging = (inputSpec, options) => {
    ...
    if (!isPrivileged) {
        ...
        spec = applyEnvToContainer(spec, CONTAINERS.WORKER, { BASE_LOGS_PATH: '/distributedkube-logs/' });
        return spec;
    }
