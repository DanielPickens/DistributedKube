...
const fsVolumes = { name: 'storage-volume', persistentVolumeClaim: { claimName: 'distributedkube-storage-pvc' } };
const fsVolumeMounts = { name: 'storage-volume', mountPath: '/distributedkubedata' };
...
describe('reconciler', () => {
    ...
    describe('reconcile algorithms tests', () => {
        ...
        it('should work with one algo', async () => {
            ...
            expect(callCount('createJob')[0][0].spec.spec.template.spec.containers[0].image).to.eql('distributedkube/worker');
            expect(callCount('createJob')[0][0].spec.spec.template.spec.containers[1].image).to.eql('distributedkube/algorithm-example');
        });
        ...
        it('should work with custom worker tag', async () => {
            ...
            algorithmTemplates[algorithm] = {
                algorithmImage: 'distributedkube/algorithm-example',
                workerImage: 'distributedkube/worker:v3.2.1-beta1',
                cpu: 2,
                mem: 400
            };
            const versions = {
            ...
            expect(res).to.eql({ [algorithm]: { idle: 0, required: 1, paused: 0, created: 1, skipped: 0, resumed: 0 } });
            expect(callCount('createJob').length).to.eql(1);
            expect(callCount('createJob')[0][0].spec.spec.template.spec.containers[0].image).to.eql('distributedkube/worker:v3.2.1-beta1');
            expect(callCount('createJob')[0][0].spec.spec.template.spec.containers[1].image).to.eql('distributedkube/algorithm-example');
        });
        ...
        it('should work with custom workerImage and registry', async () => {
            ...
            expect(callCount('createJob')[0][0].spec.spec.template.spec.containers[1].image).to.eql('my.registry/prefix/distributedkube/algorithm-example');
        });
        ...
        it('should not add Privileged flag if configured', async () => {
            ...
            expect(callCount('createJob')[0][0].spec.spec.template.spec.containers[0].volumeMounts).to.deep.include({
                name: 'logs',
                mountPath: '/distributedkube-logs/'
            });
            expect(callCount('createJob')[0][0].spec.spec.template.spec.containers[1].volumeMounts).to.deep.include({
                name: 'logs',
                mountPath: '/distributedkube-logs/'
    ...
    describe('reconcile with maxWorkers', () => {
        it('should not create job when there is ready worker', async () => {
            ...
            const algorithmImage = 'distributedkube/algorithm-example';
            const workerImage = 'distributedkube/worker';
