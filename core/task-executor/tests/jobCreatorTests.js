...
describe('jobCreator', () => {
    ...
    describe('setWorkerImage', () => {
        ...
        it('should use image from template', () => {
            ...
            const res = setWorkerImage(workerTemplate, versions);
            expect(res).to.eql('distributedkube/worker:v1.2.3')
        })
    ...
    describe('jobSpec', () => {
        ...
        it('should apply all required properties', () => {
            ...
            expect(res).to.nested.include({ 'metadata.labels.algorithm-name': 'myalgo1' });
            expect(res).to.nested.include({ 'spec.template.spec.containers[0].image': 'distributedkube/worker:latest' });
            ...
            expect(res.spec.template.spec.volumes).to.deep.include(
                {
                    ...
                    persistentVolumeClaim: { claimName: 'distributedkube-datasources' }
                }
            );
    ...
    describe('devMode', () => {
        it('should apply with devMode', () => {
            ...
            expect(res.spec.template.spec.containers[1].volumeMounts).to.deep.include(
                {
                    ...
                    mountPath: '/distributedkube/algorithm-runner/algorithm_unique_folder',
            ...
            expect(res.spec.template.spec.volumes).to.deep.include(
                {
                    name: 'distributedkube-dev-sources',
                    persistentVolumeClaim: { claimName: 'distributedkube-dev-sources-pvc' }
                }
            );
        });
