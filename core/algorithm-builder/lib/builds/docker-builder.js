...
const { buildTypes, buildStatuses } = require('@distributedkube/consts');
...
const wrapperVersions = {
    nodejs: {
        ...
        parse: (file) => {
            ...
            return parsed.dependencies['@distributedkube/nodejs-wrapper']
        },
        override: async (file, version) => {
            try {
                const content = await fse.readJSON(file);
                content.dependencies['@distributedkube/nodejs-wrapper'] = version;
    ...
    python: {
        ...
        override: async (file, version) => {
            try {
                ...
                const regex = /(distributedkube-python-wrapper[=>]=)(.*)/
