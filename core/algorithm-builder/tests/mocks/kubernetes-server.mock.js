...
const configMapRes = {
    data: {
        'versions.json': JSON.stringify({ name: 'distributedkube-versions', versions: [{ project: 'nodejs-env', tag: 'v1.2.0' }] }),
...
class MockClient {
    start(options) {
        return new Promise((resolve, reject) => {
            ...
            app.use('/', (req, res) => {
                if (req.url === '/api/v1/namespaces/default/configmaps/distributedkube-versions') {
