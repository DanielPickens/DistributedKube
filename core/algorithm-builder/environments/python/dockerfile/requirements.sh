...
# install distributedkube + dependencies
echo install distributedkube + dependencies
if [ ! -z ${PACKAGES_REGISTRY} ]; then
     echo "found pip registry ${PACKAGES_REGISTRY}. Setting trusted host to ${PACKAGES_REGISTRY_HOST}"
     pip install --trusted-host "${PACKAGES_REGISTRY_HOST}" --trusted-host "artifacts-registry" --index-url "http://artifacts-registry:8081/distributedkube/artifacts-registry/repository/python-all/simple/" --extra-index-url "${PACKAGES_REGISTRY}"  -r /distributedkube/algorithm-runner/requirements.txt
else
  pip install --trusted-host "artifacts-registry" --index-url "http://artifacts-registry:8081/distributedkube/artifacts-registry/repository/python-all/simple/"  -r /distributedkube/algorithm-runner/requirements.txt
fi
