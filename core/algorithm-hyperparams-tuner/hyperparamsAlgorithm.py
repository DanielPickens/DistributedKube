...
def start(options, distributedkubeApi):
    ...
    sharedStorage = os.environ.get('SHARED_METRICS', '/distributedkube/shared-metrics')
    ...
    def objective(trial):
        return objectiveWrapped(trial, distributedkubeApi, hyperParams, pipelineName)
...
def objectiveWrapped(trial, distributedkubeApi, hyperParams, pipelineName):
    values = {}
    for param in hyperParams:
        values[param[consts.name]] = getattr(
            sys.modules[__name__], "suggest_%s" % param[consts.suggest])(param, trial)
    pipelineResult = distributedkubeApi.start_stored_subpipeline(
