...
const PATH_PATTERN = /(?<bucket>distributedkube-index)\/(?<date>\d{4}-\d{2}-\d{2})\/(?<jobId>.*)/; // eslint-disable-line

class BaseCleaner {
    async getJobsToDelete({ indices, maxAge }) {
        const expiredIndices = indices.filter(i => isTimeBefore(i, maxAge));
        const jobsToDelete = [];
        const datesAndJobs = await Promise.all(expiredIndices.map(date => storageManager.distributedkubeIndex.list({ date })));
