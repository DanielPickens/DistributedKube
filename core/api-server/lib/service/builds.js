...
const { errorsCode } = require('@distributedkube/consts');
...
const { buildStatuses, buildTypes } = require('@distributedkube/consts');
...
class Builds {
    ...
    async _createBuildFromCode(build) {
        if (build.uploadPath) {
            await storageManager.distributedkubeBuilds.putStream({ buildId: build.buildId, data: fse.createReadStream(build.uploadPath) });
        }
        ...
        if (fileInfo || gitRepository) {
            ...
            if (shouldBuild) {
                ...
                if (fileInfo && !fileInfo.path) {
                    const filePath = storageManager.distributedkubeBuilds.createPath({ buildId: build.buildId });
