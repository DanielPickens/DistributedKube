...
const { parser } = require('@distributedkube/parsers');
const { uid } = require('@distributedkube/uid');
const { pipelineTypes, pipelineStatuses, nodeKind } = require('@distributedkube/consts');
const levels = require('@distributedkube/logger').Levels;
...
class ExecutionService {
    ...
    async _runPipeline(payload) {
        ...
        try {
            ...
            if (flowInput && Object.keys(flowInput).length && !pipeFlowInputMetadata) {
                ...
                const storageInfo = await storageManager.distributedkube.put({ jobId, taskId: jobId, data: flowInput }, tracer.startSpan.bind(tracer, { name: 'storage-put-input', parent: span.context() }));
            ...
            await storageManager.distributedkubeIndex.put({ jobId }, tracer.startSpan.bind(tracer, { name: 'storage-put-index', parent: span.context() }));
    ...
    async cleanJob(options) {
        ...
        await stateManager.updateJobStatus(statusObject);
        await Promise.all([
            storageManager.distributedkube.delete({ jobId }),
            storageManager.distributedkubeResults.delete({ jobId }),
            storageManager.distributedkubeMetadata.delete({ jobId }),
