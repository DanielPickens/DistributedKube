...
describe('Common', () => {
    ...
    describe('migrate graph', () => {
        ...
        it('should move graph from redis to db', async () => {
            ...
            await redisClient.set(`/distributedkube:pipeline:graph/${jobId2}`, JSON.stringify(graph2));

            const redisGraph = await redisClient.get(`/distributedkube:pipeline:graph/${jobId2}`);
            ...
            const redisGraph2 = await redisClient.get(`/distributedkube:pipeline:graph/${jobId2}`);
