...
const { Encoding } = require('@distributedkube/encoding');
const { request } = require('./utils');
const { uid: uuid } = require('@distributedkube/uid');
...
describe('Storage', () => {
    ...
    describe('/prefixes/:path', () => {
        ...
        it('should return two prefixes', async () => {
            const options = {
                uri: `${restPath}/${storageManager.prefixesTypes.find((p) => p === 'local-distributedkube-store')}`,
                method: 'GET'
            };
            const response = await request(options);
        ...
        it('should return zero prefixes', async () => {
            const options = {
                uri: `${restPath}/${storageManager.prefixesTypes.find((p) => p === 'local-distributedkube-metadata')}`,
                method: 'GET'
            };
            const response = await request(options);
            expect(response.body.keys).to.have.lengthOf(0);
        ...
        it('should return all prefixes by path', async () => {
            ...
            const options = {
                uri: `${restPath}/local-distributedkube-store/algorithm?${qs}`,
                method: 'GET'
            };
            const response = await request(options);
            expect(response.body.error.code).to.equal(HttpStatus.NOT_FOUND);
    ...
    describe('/keys/:path', () => {
        ...
        it('should not return specific keys', async () => {
            const alg = 'eval-alg';
            const options = {
                uri: `${restPath}/${`local-distributedkube-store/algorithm/${alg}.json`}`,
                method: 'GET'
            };
            const response = await request(options);
            expect(response.body.error.code).to.equal(HttpStatus.NOT_FOUND);
        ...
        it('should return zero keys', async () => {
            const options = {
                uri: `${restPath}/local-distributedkube-metadata`,
                method: 'GET'
            };
            const response = await request(options);
            expect(response.body.keys).to.have.lengthOf(0);
        ...
        it('should return all keys by query', async () => {
            ...
            const options = {
                uri: `${restPath}/local-distributedkube-store?${qs}`,
                method: 'GET'
            };
            const response = await request(options);
            expect(response.body.keys).to.have.lengthOf(0);
       ...
       xit('should limit the return to max keys', async () => {
            ...
            const options = {
                uri: `${restPath}/local-distributedkube`,
                method: 'GET'
            };
            const array = Array.from({ length }, (v, k) => k + 0);
            await Promise.all(array.map((a) => storageManager.distributedkube.put({ jobId, taskId: `task-${a}`, data: `data-${a}` })));
    ...
    describe('/values/:path', () => {
        ...
        it('should throw value Not Found', async () => {
            const value = 'local-distributedkube-store/algorithm/no_such_value';
            const options = {
                uri: `${restPath}/${value}`,
                method: 'GET'
            };
    ...
    describe('/stream/:path', () => {
        ...
        it('should throw stream Not Found', async () => {
            const value = 'local-distributedkube-store/algorithm/no_such_stream';
            const options = {
                uri: `${restPath}/${value}`,
                method: 'GET'
            };
    ...
    describe('/stream/custom:path', () => {
        ...
        it('should stream with custom encode: false and value: null', async () => {
            ...
            const path = storageManager.distributedkube.createPath({ jobId, taskId });
    ...
    describe('/download/pipeline/result/:jobId', () => {
        ...
        it('should succeed to get download results', async () => {
            ...
            const path1 = await storageManager.distributedkube.put({ jobId, taskId: 'a', data: 100 });
            const path2 = await storageManager.distributedkube.put({ jobId, taskId: 'b', data: 200 });
            const data = [
                {
                    nodeName: 'A',
            ...
            expect(response.response.headers['content-disposition']).to.contains("attachment; filename=distributedkube-result");
