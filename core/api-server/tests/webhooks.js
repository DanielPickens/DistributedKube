    ...
    describe('Results', () => {
        ...
        it('should succeed to send webhook result with large data', async () => {
            return new Promise(async (resolve) => {
                ...
                const response = await request(stored);
                const path1 = await storageManager.distributedkube.put({ jobId: response.body.jobId, taskId: 'a', data: 'some-path2' });
                const results = {
                    jobId: response.body.jobId,
                    status: 'completed',
                    data: [
        ...
        it('should succeed to send webhook when watch fails', async () => {
            ...
            try {
                ...
                // results.data.storageInfo = await storageManager.distributedkubeResults.put({ jobId, data: results.data });
