...
class StateManager {
        ...
        this._interval = setInterval(async () => {
            ...
            try {
                ...
                if (resources && resources[0] && resources[0].unScheduledAlgorithms) {
                    ...
                    const clusterNodes = resources[0].nodes;
                    Object.values(algorithms).forEach((e) => {
                        this._emitter.emit(`events-${e.type}`, e, clusterNodes);
                    });
                }
            }
            catch (e) {
    ...
    async setJobResultsToStorage(options) {
        ...
        try {
            if (options.data) {
                ...
                const storageInfo = await storageManager.distributedkubeResults.put({ jobId: options.jobId, data }, tracer.startSpan.bind(tracer, { name: 'storage-put', parent: span.context() }));
    ...
    _isActiveStatus(status) {
        return status !== DriverStates.PAUSED; // !== DriverStates.STOPPED && status
    }
