...
} = require('@distributedkube/datasource-utils');
...
class Repository extends RepositoryBase {
    ...
    async _enrichDvcFile(fileMeta, metaData) {
        ...
        if (dvcContent?.meta?.distributedkube?.hash) {
            if (dvcContent.meta.distributedkube.hash === dvcContent.outs[0].md5) {
                ...
                nextMeta.id = dvcContent.meta.distributedkube.id;
                nextMeta.uploadedAt = dvcContent.meta.distributedkube.uploadedAt;
            }
        }
        return this.dvc.enrichMeta(filePath, dvcContent, 'distributedkube', nextMeta);
    }
    ...
    async setup() {
        ...
        await this.createdistributedkubeFile();
    ...
    async scanDir() {
        ...
        return Promise.all(
            dvcFiles.map(async filePath => {
                ...
                const { hash, ...meta } = content.meta.distributedkube;
                return {
