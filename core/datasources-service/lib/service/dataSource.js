...
const { extractRelativePath } = require('@distributedkube/datasource-utils/lib/filePath');
    ...
    async handleUntrackedFiles(repository, directory) {
        ...
        await Promise.all(dataFiles.map(async (file) => {
            if (file) {
                ...
                await repository.dvc.enrichMeta(path.join('data', file), dvcObj, 'distributedkube', metaObj);
